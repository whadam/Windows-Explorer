...
int CSystemImageList::GetIcon(const CString& sName, BOOL bOpen /* = FALSE */) const
{
	SHFILEINFO sfi;

	if (bOpen)
		SHGetFileInfo(sName, 0, &sfi, sizeof(sfi), SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_SYSICONINDEX | SHGFI_SMALLICON | SHGFI_OPENICON);
	else
		SHGetFileInfo(sName, 0, &sfi, sizeof(sfi), SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_SYSICONINDEX | SHGFI_SMALLICON);

	return(sfi.iIcon);
}

HICON CSystemImageList::GetHIcon(const CString& sName, BOOL bOpen /* = FALSE */) const
{
	SHFILEINFO sfi;

	if (bOpen)
		SHGetFileInfo(sName, 0, &sfi, sizeof(sfi), SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_SYSICONINDEX | SHGFI_LARGEICON | SHGFI_OPENICON);
	else
		SHGetFileInfo(sName, 0, &sfi, sizeof(sfi), SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_SYSICONINDEX | SHGFI_LARGEICON);
